...
list = cwd.split('\\')
del list[-1]
list.append('BiliVideo')
download_dir = '\\'.join(list)


def login(mode):
    if mode == 'tv':
        win32api.ShellExecute(0, 'open', r'.\BBDown.exe', 'logintv', '', 0)
    else:
        win32api.ShellExecute(0, 'open', r'.\BBDown.exe', 'login', '', 0)
    time.sleep(0.5)
    root = tk.Tk()
    root.title("显示图片")
...
def run(user_input):
    ...
    if user_input.value == '':
        ...
        while True:
            icmp_out = subprocess.Popen([r".\BBDown.exe", user_input.value], shell=True,
            ...
            if "任务完成" in output:
                ...
                time.sleep(2)
                message(message=f'视频已下载到{download_dir}目录下')
                os.system(f'explorer.exe {download_dir}')
                break
            elif "输入有误" in output:
                message("BV/AV号或地址输入有误")
                break
